...
function Library:MakeNotify(config)
    ...
    -- Tinggi dihitung SEKALI di sini lewat GetTextSize. Tidak ada AutomaticSize,
    -- tidak ada koneksi TextBounds, tidak ada reflow, tidak ada tween -- setelah
    -- dibuat notifikasi ini benar-benar diam sampai dihapus.
    local W     = 300
    local PAD_L = 19     -- sisakan ruang untuk batang aksen di kiri
    local PAD_R = 14
    local PAD_T = 12
    local PAD_B = 13
    local GAP   = 5
    local textW = W - PAD_L - PAD_R
    local function measure(str, size, font)
        if str == "" then return 0 end
        local ok, h = pcall(function()
            return TextService:GetTextSize(str, size, font, Vector2.new(textW, 10000)).Y
        end)
        return (ok and h and h > 0) and h or (size + 2)
    end
    local titleH   = measure(title,   fontSize.normal, Enum.Font.GothamBold)
    local descH    = measure(desc,    fontSize.small,  Enum.Font.GothamMedium)
    local contentH = measure(content, fontSize.small,  Enum.Font.Gotham)
    local totalH   = PAD_T + titleH + PAD_B
    if descH    > 0 then totalH = totalH + GAP + descH end
    if contentH > 0 then totalH = totalH + GAP + contentH end

    local notif = new("Frame", {
        Parent = self._gui,
        Name = "LynxNotify",
        -- AnchorPoint kanan-bawah: kartu tumbuh ke ATAS saat isinya panjang,
        -- jadi tidak pernah keluar layar seperti versi tinggi-tetap 65px dulu.
        AnchorPoint = Vector2.new(1, 1),
    ...
    -- gradasi lembut: atas tetap bg2, bawah meredup. UIGradient mengalikan
    -- BackgroundColor3, jadi putih = warna asli, abu = lebih gelap.
    new("UIGradient", {
        Parent = notif,
        Rotation = 90,
        Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 255, 255)),
            ColorSequenceKeypoint.new(1, Color3.fromRGB(146, 146, 146)),
        })
    })
    -- garis kilau tipis di tepi atas, memudar di kedua ujung
    ...
    -- batang aksen kiri, memudar di ujung atas & bawah
    local accent = new("Frame", {
        Parent = notif,
        AnchorPoint = Vector2.new(0, 0.5),
    ...
    -- Label diposisikan dari tinggi yang sudah dihitung. Label kosong TIDAK
    -- dibuat sama sekali -- versi lama selalu membuat ketiganya lalu
    -- meninggalkan celah kosong kalau Description/Content tidak diisi.
    local y = PAD_T
    new("TextLabel", {
        Parent = notif,
        Text = title,
        Size = UDim2.new(0, textW, 0, titleH),
        Position = UDim2.new(0, PAD_L, 0, y),
        BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.normal,
        TextColor3 = color,
    ...
    if descH > 0 then
        ...
        new("TextLabel", {
            Parent = notif,
            Text = desc,
            Size = UDim2.new(0, textW, 0, descH),
            Position = UDim2.new(0, PAD_L, 0, y),
            BackgroundTransparency = 1,
            Font = Enum.Font.GothamMedium,
            TextSize = fontSize.small,
