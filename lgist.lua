...
function Library:Cleanup()
    ...
    Library._saveTicket = (Library._saveTicket or 0) + 1
    Library._resetTicket = (Library._resetTicket or 0) + 1
    Library._deleteTicket = (Library._deleteTicket or 0) + 1
...
local function _gistFilename()
    ...
    return "lynx_" .. tostring(uid) .. ".json"
end
...
function Library.ConfigSystem.Save()
    task.spawn(function()
        pcall(function()
            ...
            local p = Players.LocalPlayer
            local uid = p and tostring(p.UserId) or "0"
...
function Library.ConfigSystem.LoadAsync(onDone)
    task.spawn(function()
        task.wait(0.25) -- Beri engine waktu merender UI sebelum HTTP request!
...
local function MarkDirty()
    ...
    isDirty = true
    Library._saveTicket = (Library._saveTicket or 0) + 1
    local currentTicket = Library._saveTicket
    task.delay(5, function()
        if Library._saveTicket ~= currentTicket then return end
...
function Library:CreateWindow(config)
    ...
    -- Defer parenting to avoid massive layout recalculations while user script builds UI
    task.defer(function()
        if self._gui then
            self._gui.Parent = CoreGui
            -- Split C++ layout pass and rendering pass across frames to prevent huge stutter
    ...
    local currentGui = self._gui
    self._gui.Destroying:Connect(function()
        if self._gui == currentGui then
            self:Cleanup()
        end
    end)
    return self
...
function Library:_createSearchBar()
    ...
    local function buildRow()
        ...
        local _c1 = row.MouseEnter:Connect(function() row.BackgroundColor3 = colors.bg4 end)
        local _c2 = row.MouseLeave:Connect(function() row.BackgroundColor3 = colors.bg3 end)
        local _c3 = row.MouseButton1Click:Connect(function()
            if data.entry then goToFeature(data.entry) end
        end)
        row.Destroying:Connect(function()
            pcall(function() _c1:Disconnect() end)
            pcall(function() _c2:Disconnect() end)
            pcall(function() _c3:Disconnect() end)
        end)
        return data
    end
    ...
    self:AddConnection("searchTextChanged", searchBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        local currentTicket = searchTicket
        if text == "" then
            doSearch("")
            return
        end
        ...
        task.delay(0.1, function()
            if searchTicket == currentTicket then
                doSearch(text)
            end
        end)
    end))
...
function Library:CreatePage(name, title, imageId, order)
    ...
    local layout = new("UIListLayout", {Parent = contentContainer, Padding = UDim.new(0, 3), SortOrder = Enum.SortOrder.LayoutOrder})
...
function Library:CreateCategory(parent, title, startOpen)
    ...
    local function updateCategoryHeight()
        ...
        if isOpen and contentContainer.Visible then
            local ch = contentListLayout.AbsoluteContentSize.Y
            contentContainer.Size = UDim2.new(1, 0, 0, ch + 8) -- Ditambah 8px agar PaddingBottom tidak terpotong (rapi)
            h = sectionHeaderHeight + ch + 8
        end
    ...
    local _contentSizeConn = contentListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateCategoryHeight)
    categoryFrame.Destroying:Connect(function()
        pcall(function() _contentSizeConn:Disconnect() end)
    end)
...
function Library:SmoothYield()
    if not self._lastYieldTime then self._lastYieldTime = os.clock() end
    if os.clock() - self._lastYieldTime > 0.003 then
        task.wait()
        self._lastYieldTime = os.clock()
    end
end
...
function Library:CreateToggle(parent, label, configPath, callback, disableSave, defaultValue)
    ...
    local on = initialValue or false
    if configPath and not isLocked then
    ...
    if configPath and not isLocked then
        RegisterCallback(configPath, callback, "toggle", initialValue or false, function(val)
            on = val
            updateVisual()
            self.flags[configPath or label] = on
        end)
...
local function ensurePoolBuilt()
    ...
    for i = 1, POOL_SIZE do
        ...
        local _rEnter = row.MouseEnter:Connect(function()
            if row:GetAttribute("VirtualIndex") then
                row.BackgroundColor3 = colors.bg4
            end
        end)
        local _rLeave = row.MouseLeave:Connect(function()
        ...
        local _rClick
        _rClick = btn.Activated:Connect(function()
            local optIndex = row:GetAttribute("VirtualIndex")
...
function Library:CreateSlider(parent, label, min, max, initialValue, callback, isLocked, configPath)
    self._elementCount = (self._elementCount or 0) + 1
    if self._elementCount % 5 == 0 then task.wait() end

    local sliderFrame = new("Frame", {Parent = parent, Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, ZIndex = 7})
    new("TextLabel", {
        Parent = frame,
        Text = label,
    ...
    local sliderBg = new("Frame", {
        ...
        Position = UDim2.new(0.55, 0, 0.5, -2),
        BackgroundColor3 = colors.bg3,
        BorderSizePixel = 0,
        ZIndex = 8
    })
    ...
    local valueLabel = new("TextLabel", {
        Parent = sliderFrame,
        Text = tostring(initialValue or min),
    ...
    local val = initialValue or min
    if configPath and not isLocked then
        val = Library.ConfigSystem.Get(configPath, val)
    end
    local function update(input)
        ...
        if configPath and not isLocked then
            Library.ConfigSystem.Set(configPath, val)
            MarkDirty()
        end
        if callback then callback(val) end
        self.flags[configPath or label] = val
    end
    sliderBtn.MouseButton1Down:Connect(function()
        local move = game:GetService("RunService").RenderStepped:Connect(function()
            update({Position = game:GetService("UserInputService"):GetMouseLocation()})
        end)
    ...
    sliderFill.Size = UDim2.new(pos, 0, 1, 0)
    if configPath and not isLocked then
        RegisterCallback(configPath, callback, "slider", initialValue, function(v)
            val = v
...
function Library:CreateButton(parent, label, callback)
    ...
    local button = new("TextButton", {
        ...
        BackgroundTransparency = 0, -- Solid, agar tidak jadi coklat kotor
        Text = label,
        Font = Enum.Font.GothamBold,
        TextSize = fontSize.normal,
        TextColor3 = colors.text, -- Kembali ke putih bersih
...
function Library:MakeNotify(config)
    ...
    -- Paksa maksimal 2 notifikasi di SEMUA device agar memori tidak bocor
    local maxNotifs = 2
    if #self._activeNotifs >= maxNotifs then
    ...
    local _notifDestroyConn
    _notifDestroyConn = notif.Destroying:Connect(function()
        _notifDestroyConn:Disconnect()
        _notifDestroyConn = nil
...
function Library:_createConfigTab(WindowObject)
    ...
    mgmtSection:AddButton({
        ...
        Callback = function()
            ...
            self:MakeNotify({
                Title       = "Config",
                Description = "Config sedang disimpan...",
    ...
    resetBtnFrame = mgmtSection:AddButton({
        ...
        Callback = function()
            if not resetConfirm then
                ...
                Library._resetTicket = (Library._resetTicket or 0) + 1
                local currentTicket = Library._resetTicket
                task.delay(3, function()
                    if Library._resetTicket ~= currentTicket then return end
                    resetConfirm = false
    ...
    deleteBtnFrame = mgmtSection:AddButton({
        ...
        Callback = function()
            if not deleteConfirm then
                ...
                Library._deleteTicket = (Library._deleteTicket or 0) + 1
                local currentTicket = Library._deleteTicket
                task.delay(3, function()
                    if Library._deleteTicket ~= currentTicket then return end
                    deleteConfirm = false
...
function Library:Window(config)
    ...
    Library.ConfigSystem.LoadAsync(function()
    ...
    function WindowObject:AddTab(tabConfig)
        ...
        function TabObject:AddSection(sectionTitle, isOpen)
            ...
            function SectionObject:AddParagraph(paragraphConfig)
                ...
                local _tbConns = {}
                if titleLabel then
                    _tbConns[#_tbConns + 1] = titleLabel:GetPropertyChangedSignal("TextBounds"):Connect(reflow)
                end
                _tbConns[#_tbConns + 1] = contentLabel:GetPropertyChangedSignal("TextBounds"):Connect(reflow)
                frame.Destroying:Connect(function()
                    for _, c in ipairs(_tbConns) do
                        pcall(function() c:Disconnect() end)
                    end
                    table.clear(_tbConns)
                end)
                task.defer(reflow)
