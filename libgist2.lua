...
function Library:Cleanup()
    ...
    if self._configResetThread then
        pcall(function() task.cancel(self._configResetThread) end)
        self._configResetThread = nil
    end
    if self._configDeleteThread then
        pcall(function() task.cancel(self._configDeleteThread) end)
        self._configDeleteThread = nil
    end
    ...
    resetBtnFrame = mgmtSection:AddButton({
        ...
        Callback = function()
            if not resetConfirm then
                ...
                if Library._configResetThread then pcall(function() task.cancel(Library._configResetThread) end) end
                Library._configResetThread = task.delay(3, function()
                    Library._configResetThread = nil
                    resetConfirm = false
    ...
    deleteBtnFrame = mgmtSection:AddButton({
        ...
        Callback = function()
            if not deleteConfirm then
                ...
                if Library._configDeleteThread then pcall(function() task.cancel(Library._configDeleteThread) end) end
                Library._configDeleteThread = task.delay(3, function()
                    Library._configDeleteThread = nil
                    deleteConfirm = false
